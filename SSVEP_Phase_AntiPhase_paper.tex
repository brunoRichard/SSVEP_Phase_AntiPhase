...
\includegraphics{Figures/MethodsFigure.png}
...
\includegraphics{SSVEP_Phase_AntiPhase_paper_files/figure-pdf/fig-SNRData-1.pdf}
...
\includegraphics{SSVEP_Phase_AntiPhase_paper_files/figure-pdf/fig-SNRComparison-1.pdf}
...
\includegraphics[width=0.7\textwidth,height=\textheight]{Figures/FullModel.png}
...
harmonics (Wade and Baker, 2025). In a linear sum, stimuli presented
...
responses at harmonic frequencies,
...
\includegraphics{SSVEP_Phase_AntiPhase_paper_files/figure-pdf/fig-badModels1-1.pdf}
...
binocular stage (Meese et al., 2006; Meese and Baker, 2011),
...
excitatory component and determines the extent of summation at detection
threshold. The monocular input also undergoes self-suppression before
being fed into the binocular stage. Unlike the original two-stage
contrast gain control model, this iteration does not include an
inter-ocular suppression term, which impacts the model's performance
(see Table~\ref{tbl-R2Table}). The suppression at the monocular stage is
not large enough to reduce responses at the fundamental frequency
(Figure~\ref{fig-betterModels}). While it does generate responses for
stimuli presented in temporal anti-phase, as the rectification prevents
their sum from cancelling, they are too small in magnitude to capture
observer SNRs.
...
\includegraphics{SSVEP_Phase_AntiPhase_paper_files/figure-pdf/fig-betterModels-1.pdf}
...
\includegraphics{SSVEP_Phase_AntiPhase_paper_files/figure-pdf/fig-bestModels-1.pdf}
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.4845}}
