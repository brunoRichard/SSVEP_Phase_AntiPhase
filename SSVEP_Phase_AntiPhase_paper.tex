...
Figure~\ref{fig-sumDiagram} \textbf{maybe in the appendix?}). The
...
\includegraphics[width=0.5\linewidth,height=\textheight,keepaspectratio]{Figures/SumDiagram.png}
...
(\(\approx 30\)) exponent. This method of signal combination was not
appropriate for our data, because in SSVEP paradigms the recorded signal
measured at the scalp is the pooled activity of all responsive neurons.
In the model, the binocular response is always larger than the monocular
response and thus dominates in the final signal, preventing a model with
MAX pooling from capturing the fundamental and odd-harmonics responses
for anti-phase stimuli. Instead, we found that a linear combination of
the binocular signal and the monocular signals,
...
use the monocular response from the left eye (\(R_{\mu_L}\)) to add to
the binocular channel. {[}Not sure I understand this bit - why not add
both monocular channels?{]} Adding parallel monocular channels to the
...
The code in this temporary section explores how different methods of
combining the monocular and binocular signals impacts the fits to our
data. The ``original'' binocular plus left eye alone is used as a
reference model and explains 89.4\% of the variance. Adding the left and
right eye signals to the binocular inputs drops the model performance to
82.4\% (similar to the two-stage model without monocular channels). This
performance drop can be explained by the right and left eye signals
cancelling themselves out when presented under temporal anti-phase,
removing the monocular responses (3Hz,9Hz,15Hz) from the model output.
Using a Minkowski sum in an effort to preserve the monocular signals
does not improve fits. Summing the Fourier amplitudes does improve fits
but is not really a plausible model. Finally, using the difference
signal between the left and right eye brings performance back to the
same level as the ``original'' model that only uses the left eye output.
...
responses that are quite similar to the temporal in-phase stimuli and
overestimates the SNRs for monocular frequencies but fits the binocular
frequencies better (particularly for spatial and temporal anti-phase).
...
\endlastfoot
Bin + Left & 0.894 \\
Bin + Left + Right & 0.824 \\
Bin\^{}w + Left\^{}w + Right\^{}w & 0.829 \\
fft(Bin) + fft(Left) + fft(Right) & 0.862 \\
Bin + (Left-Right) & 0.894 \\
Bin + Left + Left & 0.902 \\
\end{longtable}

Fits of the Bin + Left model and the Bin + (Left-Right Model)

\pandocbounded{\includegraphics[keepaspectratio]{SSVEP_Phase_AntiPhase_paper_files/figure-pdf/unnamed-chunk-13-1.pdf}}
