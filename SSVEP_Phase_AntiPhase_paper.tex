...
are not identical) stimuli, in a two-stage process. First, the input is
rectified by an excitatory non-linearity (\(m \approx 1.3\)) and
normalized by self and interocular suppression (see
Equation~\ref{eq-intoMath2}). The binocular (e.g., combined) input
...
responses; it does not preserve monocular signals beyond the first
...
original (6Hz) and its integer harmonics (2F-12Hz and 3F-18Hz). The
responses we recorded at the fundamental frequency (3Hz) and its odd
integer harmonics (3F - 9Hz, 5F - 15Hz) cannot be explained by ab
architecture with a purely binocular output. Next, we explore methods of
preserving the monocular signal in an effort to explain observer
...
monocular channels improved the fits to our data (see
...
accounted for in their computational description. Parallel monocular
channels, as defined by Georgeson et al. (2016) appear to be an adequate
...
to account for these effects. In our study, we find little evidence of
any influence of spatial phase on observer SSVEPs as no statistically
significant difference in signal-to-noise ratios across spatial phase
were found. Still, we opt it prudent to verify if the addition of
phase-selective channels improves the fits of the model to our data.
Phase-selectivity was added to the binocular and monocular channels by
replicated the equations of the first and second stage, for a total of
six channels (see Figure~\ref{fig-modelDiagram}). As with the previous
model, no additional model parameters were required to include
phase-selectivity; the parameters \(m\), \(p\), \(q\), \(S\) and \(Z\)
used to define the responses of all six channels in this model.
...
its variants - to capture our neural data. No statistically significant
difference was found between the monocular and binocular conditions
presented either with On/Off or counterphase flicker. This is consistent
with ocularity invariance; binocular and monocular stimuli are judged
equal in magnitude at high contrast (Baker et al., 2018; Legge, 1984;
Maehara and Goryo, 2005; Meese et al., 2006). Presenting stimuli in
temporal anti-phase reduced the magnitude of responses at the
...
under binocular viewing, monocular signals can be measured at the scalp
with SSVEPs. This was confirmed in our modelling as only the two-stage
contrast gain control model with parallel monocular channels, which
preserve monocular signals throughout the model architecture, was able
to accurately capture the data in all experimental conditions. Simpler
models failed to generate the necessary responses at the fundamental and
odd-integer harmonics for stimuli presented in temporal anti-phase while
a more complex model that included phase-selectivity did not improve the
quality of fits. Overall, we find that the same general framework that
has been used to explain psychophysical stimulus combination can be
successfully applied to neural data collected under a variety of
...
et al., 2015). Phase dependent effects in SSVEPs have been recorded with
motion stimuli (Cottereau et al., 2014; Kohler et al., 2018). The
dichoptic presentation of moving bars, whereby they either move in-phase
(lateral motion) or in anti-phase (motion-in-depth) impacts the
amplitude of SSVEPs. In-phase motion generates SSVEP amplitudes that are
twice those of motion in anti-phase (Cottereau et al., 2014). Why we do
not observe phase-dependent effects in our study may reflect a
limitation of the SSVEP technique: SSVEPs recorded at the scalp are a
spatial aggregation of the stimuli presented to observers (Wade and
Baker, 2025). Spatially aggregated responses will be identical
regardless of stimulus phase and thus generate phase-insensitive
responses.

\paragraph{Difference channels}\label{difference-channels}

An aspect that we do not explore in this study is the potential
contribution of differencing channels to observer data. Visual inputs
are summed and their difference is taken.

Henriksen and Read (2016) May and Zhaoping (2016)
...
\bibitem[\citeproctext]{ref-Henriksen2016}
Henriksen, S., Read, J.C.A., 2016. Visual perception: A novel difference
channel in binocular vision. Curr Biol 26, R500--R503.
\url{https://doi.org/10.1016/j.cub.2016.04.049}
