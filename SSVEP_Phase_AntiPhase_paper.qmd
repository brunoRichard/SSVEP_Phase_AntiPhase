...
```{r, echo = FALSE}
R2 <- NULL
SNRAll <- array(NA, dim = c(length(theFs), dim(CL.Plus)[2],5))
```
...
for (i in 1:9){
  ...
  lines(1:6, SNRAll[,i,1], col = colours[1], lwd= 2)
  lines(1:6, SNRAll[,i,5], col = colours[2], lwd = 2)

  mtext(theConditions[i], 3, line = 0.4, cex = .8, adj = 0, padj = 0, font = 2)
  
  if (i == 1){
    legend("topleft", legend = c("Bin + Left", "Bin + (Left-Right)"), col = colours[c(1,2)], lwd = 2, bty = "n")
  } else if (i == 5){
    legend("topright", legend = c(bquote("R"^2~"="~.(round(R2[1],3))), bquote("R"^2~"="~.(round(R2[5],3)))), col = colours[c(1,2)], lwd = 2, bty = "n")
  }
}
```
